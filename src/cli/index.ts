...
export function parseCommandLineArgs(): CLIOptions {
  ...
  program
    .description(
      "Track lines of code over time by language with granularity, top-N, and repo path.",
    )
    .option("-p, --path <path>", "Path to the Git repository", ".")
    .option("-s, --step <number>", "Sample every Nth commit", "1")
    .option(
      "-g, --granularity <type>",
      "Granularity: commits | daily | weekly | monthly",
      "commits",
    )
    .option(
      "-t, --top <number>",
      "Limit to top N languages by total lines",
      "0",
    )
    .option(
      "-e, --exclude <languages>",
